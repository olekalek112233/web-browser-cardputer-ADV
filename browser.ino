...
bool isFrogFindUrl(const String& url) {
  String lower = lowerCopy(url);
  return lower.startsWith("http://frogfind.com") || lower.startsWith("https://frogfind.com") ||
         lower.startsWith("http://www.frogfind.com") || lower.startsWith("https://www.frogfind.com");
}

bool isWikipediaUrl(const String& url) {
  String lower = lowerCopy(url);
  return lower.indexOf("wikipedia.org") >= 0 || lower.indexOf("wiktionary.org") >= 0 ||
         lower.indexOf("wikimedia.org") >= 0;
}

char fromHexNibble(char c) {
  ...
  for (size_t i = 0; i < encoded.length(); ++i) {
    ...
    if (c == '%' && i + 2 < encoded.length()) {
      ...
      if (isxdigit(static_cast<unsigned char>(hi)) && isxdigit(static_cast<unsigned char>(lo))) {
        decoded += static_cast<char>((fromHexNibble(hi) << 4) | fromHexNibble(lo));
        i += 2;
        continue;
      }
    }
...
String extractFrogFindReadTarget(const String& url) {
  int paramPos = url.indexOf("a=");
  if (paramPos < 0) return "";

  String value = url.substring(paramPos + 2);
  int ampPos = value.indexOf('&');
  if (ampPos >= 0) value = value.substring(0, ampPos);
  return urlDecode(value);
}

bool isFrogFindIncompleteHtmlPage(const String& html) {
  String lower = lowerCopy(html);
  return lower.indexOf("invalid or incomplete html") >= 0 ||
         lower.indexOf("had a problem getting the article") >= 0;
}

String narrowHtmlForParsing(const String& html, const String& url) {
  if (!isWikipediaUrl(url)) return html;

  String lower = lowerCopy(html);
  int start = lower.indexOf("id=\"mw-content-text\"");
  if (start < 0) start = lower.indexOf("mw-parser-output");
  if (start < 0) return html;

  int divStart = lower.lastIndexOf("<div", start);
  if (divStart >= 0) start = divStart;

  int end = lower.indexOf("<div id=\"catlinks\"", start);
  if (end < 0) end = lower.indexOf("<div class=\"printfooter\"", start);
  if (end < 0) end = lower.indexOf("<div id=\"footer\"", start);
  if (end < 0) end = lower.indexOf("</main>", start);
  if (end < 0 || end <= start) return html.substring(start);

  return html.substring(start, end);
}

String navigationUrlForLink(const String& linkUrl) {
  if (isFrogFindUrl(linkUrl)) return linkUrl;

  if (!isFrogFindUrl(g_currentUrl)) {
    return linkUrl;
  }

  if (isWikipediaUrl(linkUrl)) {
    return linkUrl;
  }

  return frogfindProxyUrl(linkUrl);
}
...
String httpGet(const String& url) {
  const bool isHttps = lowerCopy(url).startsWith("https://");
  ...
  if (isHttps) {
    ...
    http.addHeader("User-Agent", "M5Cardputer FrogFind Browser/1.0");
...
void openUrl(const String& url, bool pushHistory = true) {
  ...
  g_currentUrl = activeUrl;
  setStatus("Pobieram...");

  String body = httpGet(activeUrl);
  if (body.isEmpty()) {
    g_loading = false;
    drawUi();
    return;
  ...
  if (isFrogFindUrl(activeUrl) && isFrogFindIncompleteHtmlPage(body)) {
    String fallbackUrl = extractFrogFindReadTarget(activeUrl);
    if (!fallbackUrl.isEmpty()) {
      setStatus("FrogFind nie dal rady, proboje bezposrednio...");
      String fallbackBody = httpGet(fallbackUrl);
  ...
  body = narrowHtmlForParsing(body, activeUrl);
  g_page = parseHtmlPage(body, activeUrl);
  g_scrollLine = 0;
  wrapCurrentPage();
  g_loading = false;
...
void handleBrowserCommand(String cmd) {
  ...
  if (isNumberOnly(cmd)) {
    ...
    openUrl(navigationUrlForLink(link.url));
    return;
  }
